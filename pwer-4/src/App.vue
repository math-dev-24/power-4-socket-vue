<template>
  <div>
    ...
    >
      <li
          v-for="game in games"
          class="border rounded-lg p-4 mx-2 flex gap-4"
    ...
    <div v-if="inGame && actualGame" class="border container mx-auto rounded py-6 px-4 drop-shadow bg-slate-50 mt-10">
      ...
      <div class="flex items-center gap-1 mt-2 text-yellow-600">
        <span>Joueur 1 : </span>
        <span class="font-bold uppercase">{{ actualGame.players[0].name }}</span>
      </div>
      <div v-if="actualGame.players.length === 2" class="flex items-center gap-1 text-red-700">
        <span>Joueur 2 : </span>
        <span class="font-bold uppercase">{{ actualGame.players[1].name }}</span>
      </div>
      <div v-if="actualGame.state === GameState.STARTED" class="text-xl font-bold my-2">
        Au tour de : {{ actualGame.currentPlayer?.name }}
      </div>
      ...
      <Plateau :board="actualGame.board" @cellClicked="cellClicked" v-if="actualGame.state === GameState.STARTED || actualGame.state === GameState.FINISHED"/>
